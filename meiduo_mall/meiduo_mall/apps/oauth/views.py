...
#  / oauth / qq / authorization/?state=xxxxxxx
...
class OAuthQQURLView(APIView):
    """
    提供QQ登陆的网址
    前段请求的接口网址：/oauth/qq/authorization/?state=xxxxxxx
    state参数是由前端传递，参数值为在QQ成功登陆后，我们吧用户引导到哪个页面
    """
    def get(self, request):
        #提取state参数
        state = request.query_params.get('state')
        if not state:
            state = '/' #若前端未指明，登陆成功后则跳转到主页
        # 按照QQ的说明文档，拼接用户的QQ登陆的连接地址
